...
echo "家目录：$HOME"
echo "构建日志：$BUILD_LOG"
echo "山河烬 FE8 改版环境搭建 —— $(date '+%F %T')" >> "$BUILD_LOG"
...
echo "完整输出同时写入：$BUILD_LOG"
set +e
run_logged make -j"$(nproc)"
rc=$?
set -e
[ "$rc" -eq 0 ] && ok "make 返回 0" || warn "make 返回 $rc（先看下面有没有产物）"

if ls build/**/*.gba >/dev/null 2>&1 || find build -name '*.gba' -print -quit 2>/dev/null | grep -q .; then
  ok "默认构建成功"
else
