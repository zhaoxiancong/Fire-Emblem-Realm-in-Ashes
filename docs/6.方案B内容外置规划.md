...
### 1.2 框架仓库（只读依赖，不提交、不推送）

```
~/projects/fireemblem8-expansion/     # 由 framework.lock 钉死在 ec1dc855
├── src/data/     ← 被 content/data/ 覆盖
├── texts/        ← 被 content/texts/ 覆盖
...
| 2 | **铺设**：`content/data/*.json` → 框架 `src/data/`；`content/texts/` → 框架 `texts/`；`content/src/*.c` → 框架 `src/` | 逐文件校验落地结果 |
| 3 | **构建**：调 `make`（含宿主机工具保障） | 失败则打印日志路径与 grep 命令 |
| 4 | **验产物**：ROM 存在 + boot-check 通过 | 否则 exit 1 |
| 5 | **反查**：`git -C 框架 status --porcelain` 是否有未纳管改动 | 有则高亮警告（可能是手滑直接改了框架） |

### 2.2 关键设计约束

- **幂等**：可重复跑。铺设用覆盖 + 追加，不清空目录、不删上游文件。
- **不静默**：每一步打印实际做了什么（`[copy] xxx.json`、`[skip] 无变化`）。
- **可预演**：`DRY_RUN=1` 只打印将要做什么，不落盘。
- **反查是防呆关键**：这是第 5 步存在的全部理由——它能在你忘记"应该改 content/"时立刻抓到。
...
- [ ] 1.3 写 `tools/shanhe-build.sh`（五步 + 合并逻辑 + DRY_RUN）
