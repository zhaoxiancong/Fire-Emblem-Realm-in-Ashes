...
### 3.4e ★★ 修正：`indexed.txt` **不进 ROM** —— 补上真正生效的消息表通道（**2026-09-25 决定性修正**）

> **§3.4d 有一个致命遗漏，本节修正它。** §3.4d 把 `indexed.txt` 说成"中文文本覆盖通道"，实测证明：**它根本不进 ROM**。只按 §3.4d 做，结果是"审计全绿、报告漂亮、ROM 里还是英文"。
...
| **B · 消息表** | `texts/texts.txt` | **被 Makefile 编译进 ROM**（`Makefile:562` → `src/msg_data.c` → Huffman 压缩） | ✅ **进** | FE8U **target id**（`## MSG_<HEX>`） |

**怎么发现的**：做完 §3.4d 的全部覆盖（审计全绿）后，在 ROM 里搜 UTF-8「虞聪」→ **搜不到**。进一步查 `src/msg_data.c`（`texts.txt` 的编译产物）→ 仍是 `Eirika[X]`。**结论：§3.4d 的通道 A 对 ROM 毫无影响。**

> 附带一课：**ROM 里的文本是 Huffman 压缩的**，即使通道 B 生效了，搜 UTF-8 明文也搜不到。验证方法见 `5.改版工程手册.md` §5.4。

**通道 B 的内容外置补丁**：`content/texts/msg_overrides.zh-Hans.json`
...
**实测证据（字节级，硬）**：Huffman 解压 `src/msg_data.c` 得 `## MSG_212: '虞聪<00>'`；压缩字节在最终 ROM 中各命中 **1 次**。详见 `5.改版工程手册.md` §5.4。
