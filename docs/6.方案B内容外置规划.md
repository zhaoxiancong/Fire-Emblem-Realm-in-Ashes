...
| **v1.17** | **★ 定位 uimenu 崩溃的源码级根因 + 补丁迭代至 v4**：教学章 `DISABLEOPTIONS(~EVENT_MENUOVERRIDE_ATTACK)`（隐藏除攻撃外全部，含待機）叠加 `src/bmmenu.c:2451` `AttackCommandUsability` 在 `US_HAS_MOVED` 时返回 `MENU_NOTSHOWN` ⇒ **可见项 = 0**（合法状态）；`uimenu.c` 的 `Menu_Setup` 允许产出 `itemCount==0` 且 `menuItems[]` 未写入，消费端 8 处无有效性检查 ⇒ 野指针。**v4 引擎层根因修法**：`itemCount == 0` 时 `Proc_End` 结束该菜单；其余 4 处为防御纵深。同时把 `characters.json.base` 修正为「设计值 − 职业基值」的增量（原先与职业基值重复相加）|
| **v1.18** | **✅ 崩溃问题收口：回退引擎层补丁，改用内容层修法（实机通过）**。统一规则＝凡 `DISABLEOPTIONS(~EVENT_MENUOVERRIDE_X)`（X≠WAIT）一律补 `& ~EVENT_MENUOVERRIDE_WAIT`，永远保留「待機」⇒ 菜单不可能为零项。改动 5 处（prologue 2 / ch1 3）。删除 `uimenu-empty-menu-guard.patch`（v1→v5 引擎层补丁），`src/uimenu.c` 已还原上游原样；反查白名单 `src/uimenu.c` → `src/events/*.h`。另修：`调试启动-GDB.cmd` 的 `start` 静默失败（改用 Start-Process）＋ mGBA 带 `-g` 会停在复位等调试器（现自动接上捕手 continue）|
| **v1.19** | **§1.1 结构图同步：`tools/` 17 → 29 个文件**，并指向新增的 `tools/README.md`（按「我想做什么」索引的分类说明：日常主入口 / 环境搭建 1~7 / WSL 专有 / 中文文本验证 / 字库补字管线 / 实机调试与崩溃定位 / 命名约定）。同时登记一处冗余：`tools/wsl/` 下 5 个文件与根级**内容完全相同**，建议保留根级后删除副本（需先确认无脚本按 wsl/ 路径调用）|
| **v1.20** | **清理 `tools/wsl/` 下 5 个与根级逐字节相同的副本**（2-setup-project.sh / 3-verify-env.sh / 6-diagnose-build.sh / 7-fix-and-build.sh / shanhe.sh），并同步 8 处引用（docs/5 ×3、tools/7-fix-and-build.sh 注释 ×3、tools/shanhe.sh 注释、docs/6/README 结构图）。理由：副本的"路径转义"借口不成立（`tools/wsl/` 同样含中文+空格），正确做法是用双引号包住源路径。此后 `tools/wsl/` 只放"必须在 WSL 内运行且根级无同名"的工具。另：`tools/` 计数 29 → 24 |
| **v1.21** | **§5 回填 M3① 实测结论并勘误旧表述**：① 撤销"剑中立"的待办（已定案为**刀剑一家**，见 `3.游戏数值设定.md` §2.0）；② 更正"若某类型不参与任何环，可能需要在 schema 层面小改"——实测 **schema 本就放行环外类型**（它只约束"被写进规则的类型"必须属于两环），⇒ **中立零成本**；③ 补记物理环「零改动」与魔法环「6 条反向」的落地链路（`WHOLE_TABLE_REPLACE` + 参考块框架补丁）|
| **v1.22** | **§3.4g 框架补丁通道加固（2026-09-25 实测缺陷）**：① **修复"同文件多补丁互相覆盖"**——旧 3c'' 对每个补丁各自 `git checkout HEAD -- 目标`，后者的 checkout 会把前一个补丁的成果**整个抹掉**（日志上两个都报成功 = 静默丢补）；改为**两阶段**（先统一还原去重目标，再按序 apply）+ 末尾「应用数 == 预期数」校验。暴露条件正是本次新增的 `shenqi-no-weapon-triangle.patch` 与既有 `weapontriangle-magic-ring.patch` 同改 `src/bmbattle.c`。② **第 6 步反查白名单改为动态**——补丁目标从 `content/framework-patch/*.patch` 实时解析，不再写死在 `case` 列表里（否则每加一个改新文件的补丁就要手改白名单，漏改会噪音化告警）。③ 背景段与补丁清单同步为当前 5 个补丁 |
...
**补充：无定位键的表 —— 显式声明「整表替换」（M3① 起启用）**

少数表是**纯规则表**（如 `weapontriangle.json`）：没有 `characterId` 这类稳定标识键，逐条定位没有意义。这类表**必须在脚本里显式登记**，走整表替换：

```python
WHOLE_TABLE_REPLACE = ("weapontriangle.json",)
```

- 登记后，补丁里的数组**整体取代**框架原表（实现见 `tools/shanhe-build.sh:386-406`）。
- **刻意做成显式白名单**，不做"找不到键就整表覆盖"的兜底 —— 否则一旦定位键写错，会被静默当成整表替换，把框架自带内容冲掉。
- 未登记、又找不到标识键的表：脚本**报错并给出提示**（"若该表本就无标识键（纯规则表），请把 'X' 加入 `WHOLE_TABLE_REPLACE` 走整表替换"）。
...
> **背景**：本通道最初为修教学关卡 `Menu_OnInit` 野指针崩溃而开（根因链见 `5.改版工程手册.md` §5.7）——
> 那是 `src/uimenu.c` 的代码缺陷，数据层修不了。
> 后来发现**内容层能消灭触发前提**（教学关保留「待機」），`uimenu.c` 补丁随即回退，
> 但通道保留了下来 —— 因为之后又冒出三类**确实只能在框架侧改**、且数据层表达不了的需求：
>
> | # | 需求 | 为什么数据层修不了 |
> |---|---|---|
> | ① | 三才法环方向 | round-trip 强制「生成产物 == 手写参考块」，而参考块在 `#if !GENERATED_DATA_…` 里（改它零编译影响） |
> | ② | 神器「不入三环」 | 需要一个**新道具属性位**（`include/bmitem.h` 枚举），JSON 里写不出来 |
> | ③ | 三角生效处的守卫 | 属性位只是"标记"，**清零动作必须在 `BattleApplyWeaponTriangleEffect()` 里**（见 docs/5 §5.8） |
>
> **这是方案 B 铁律①「框架里不出现手改」的已知、且已登记的例外。**
...
# shanhe-build.sh 第 3c'' 步自动执行（**两阶段**）：
#   阶段 1) 收集所有补丁的目标文件、**去重**，每个目标 `git checkout HEAD -- <目标>` 还原一次
#   阶段 2) 按文件名顺序 apply 全部补丁（此处不再 checkout）
#   apply 失败 → die（不继续构建）；末尾校验「实际应用数 == 预期数」
# 第 2 步「写前快照」与第 6 步「反查白名单」都已纳入补丁目标（含 `include/*.h`）
```

> ⚠️ **必须是两阶段，不能"每个补丁前各自 checkout"**（2026-09-25 实测发现的缺陷）：
> 同一文件有多个补丁时，后者自己的 `checkout` 会**把前一个补丁的成果整个抹掉**，
> 而日志上两个都报「已应用」—— **静默丢补**。加第二个改 `src/bmbattle.c` 的补丁时暴露。
> 末尾的补丁数校验是第二道闸：防循环体某次 `continue` 静默漏补（教训同 docs/5 §5.1）。

**当前补丁清单**（2026-09-25 最终态，5 个 → 4 个目标文件）：
...
| `weapontriangle-magic-ring.patch` | `src/bmbattle.c` | 魔法环手写参考块反向（三才法环；否则 round-trip 报 6 diagnostics） |
| `shenqi-attribute-bit.patch` | `include/bmitem.h` | 新增 `IA_SHENQI = 1<<22`（神器属性位） |
| `shenqi-no-weapon-triangle.patch` | `src/bmbattle.c` | 神器不入三环守卫（与上一个配套，缺一无效） |

> `weapontriangle-magic-ring` 与 `shenqi-no-weapon-triangle` **同改 `src/bmbattle.c`**
> （上下文相隔约 1900 行，互不重叠）—— 这也是上面"两阶段"缺陷的唯一触发条件。
...
**这对本项目的意义**：
- 你的**兵刃三环**（枪克刀剑克锤克枪）按映射折算后与 `GROUPS[0]` **逐条一致** ⇒ **物理环零改动**。
- 你的**三才法环**（天克地克人克天）对应 `GROUPS[1]`（魔法环）→ 需把 6 条正负号**反向**。
- **✅ 实测结论（M3①，2026-09-25）**：`GROUPS` 确是硬编码的封闭列表，但**它只约束「被写进规则的类型」必须属于两环，从不要求每个类型都必须入环** —— 对"不参与任何环"的类型（如 `ITYPE_BOW`）**本就放行**。⇒ **"中立"零成本**，这正是采用「弓 / 弩 = 中立位」的依据（剑则并入刀，见 `3.游戏数值设定.md` §2.0 决策记录）。
- **已落地**：`content/data/weapontriangle.json`（整表替换，走 `shanhe-build.sh` 的 `WHOLE_TABLE_REPLACE`，见 §3.4）+ `content/framework-patch/weapontriangle-magic-ring.patch`（改 `src/bmbattle.c` 的 `#if !GENERATED_DATA_WEAPONTRIANGLE_LINKED` 参考块；不改则 round-trip 报 `6 diagnostic(s)`）。
