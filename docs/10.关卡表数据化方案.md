...
> **文档版本**：v1.3
> **编写日期**：2026-09-25
> **文档定位**：回答 **D-6「关卡表怎么产出」**——把 `1.剧情与世界观设定.md` §4.5/§4.6 的逐章剧情与地图设计，转成能进框架数据管线的 **JSON 关卡数据**。
>
> **与其它文档的分工**：
>
...
| --- | --- | --- | --- |
| **具体做法** | 新建 `docs/4.关卡表_Demo五章.md`（或复号），逐章列表 | 在 `1.剧情与世界观设定.md` 加一节关卡表 | 关卡设计的**人读部分**留文档 1 §4.5/§4.6；**机读部分**直接写 `content/data/chN_*.json` |
...
**已经在文档 1 §4.5（序章~第 4 章）+ §4.6（第 5~20 章 + 2 外传）里了。** 每章都有：
...
> **为什么随章做**：因为**关卡设计会在制作中迭代**。提前把 20 章 JSON 全写好，等制作到第 12 章时早就过时了。文档 1 §4.5/§4.6 提供"设计意图"作为足够稳定的上游，JSON 按需产出。
...
| # | 文件 | 内容 | 上游依据 |
| --- | --- | --- | --- |
| 1 | `chN_units.json` | 我方出场 + 敌方配置 + NPC | 文档 1 §4.5/§4.6【战场与目标】【BOSS】 |
| 2 | `chN_shops.json` | 武器店/道具店 | 文档 1 §4.5/§4.6 + 数值文档 |
| 3 | `chN_traps.json` | 陷阱、地形伤害（第 9 章灰潮、第 6 章火场） | 文档 1 §4.5/§4.6【地图设计】 |
| 4 | `chN_eventscripts.json` | 开场对话、中场事件、BOSS 对话、战后、宝箱 | 文档 1 §4.5/§4.6【开场】【中场事件】【战后】 |
...
### 6.4 ☠️ 章节目标钩子的能力边界（**做关卡目标前必读**）

框架提供了一个**数据驱动**的章节目标扩展点，但它的射程**比直觉窄得多**。

**入口**：`include/expansion_chapter_objectives.h` + `src/data/chapter_objectives.json`。

**容量**：bundle 32 / 每章 8 / 组 8×16（`expansion_chapter_objectives.h:42-45`）。

| ✅ 只支持这 5 种目标类型 | ❌ 不支持 |
|---|---|
| `protect`（保护） | **`seize`（占领王座/城门）** |
| `reach_area`（到达区域） | **村庄访问** |
| `defeat_group`（击破某组） | **宝箱** |
| `event_flag`（事件旗标） | 路线分支 |
| `hold_until_turn`（坚守到某回合） | 目标优先级、玩家可见文本 |

**另外两条硬限制**：

1. **激活条件只能由既有事件旗标推导**（`docs/autoplay.md:395-427`），不能自定义复杂条件。
2. **它不产生玩家可见的目标描述文本**——"胜利条件"要显示什么，仍须另走文本通道（见 `5.改版工程手册.md` §4.4）。

> **⇒ 对《山河烬》20 章的含义**：
> **"占领/制压"类目标、村庄访问、宝箱这三类，钩子完全帮不上**，必须落回**事件脚本**（`src/events/*.c`，即本文档 §3.1 说的"章节制作唯一需写代码处"）。
> 设计关卡时**先按这 5 类筛一遍**，凡不在其中的，预算里直接算作"写事件脚本"的工作量。
...
| 项 | 定案前 | 定案后 |
| --- | --- | --- |
| M4 备注 | 「关卡表缺失问题必须在此前解决」（§2.4 第 6 项） | ✅ **已解决**：设计在文档 1 §4.5/§4.6，数据按本文档 §3 产出 |
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| **v1.3** | **2026-09-25** | **新增 §6.4「章节目标钩子的能力边界」**（依据外部研读报告，快照同为 `ec1dc855`）：框架的 `expansion_chapter_objectives` 只支持 **5 种目标类型**（protect / reach_area / defeat_group / event_flag / hold_until_turn），**不支持 seize（制压）、村庄访问、宝箱**；且激活条件只能由既有事件旗标推导、不产生玩家可见目标文本。**⇒ 这三类目标必须落回事件脚本**（`src/events/*.c`），设计关卡时先按 5 类筛一遍 |
| v1.2 | 2026-09-25 | 同步文档 1 §4.5 重写（Demo 五章补齐【序章旁白】【地图设计】，与 §4.6 同构）：全文 9 处「文档 1 §4.6」引用改为「§4.5/§4.6」 |
