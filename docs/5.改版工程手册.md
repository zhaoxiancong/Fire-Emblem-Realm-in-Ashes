...
- **已实施（2026-09-25）**：走**新增的「框架补丁通道」**打最小本地补丁
  ...
  **v4 引擎层根因**：`itemCount == 0` 时 **`Proc_End` 直接结束该菜单**，让"零项菜单"这个非法状态不存在）。
  v4 的 5 处改动：
  ...
  4. **（根因）`Menu_OnInit`：`itemCount == 0` 时 `Proc_End(proc)` 结束该菜单**（+ `itemCurrent` 夹取）；
