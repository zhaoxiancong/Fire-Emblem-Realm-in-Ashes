...
> 状态：**v0.3，环境与构建全链路实机跑通**。已产出 16M 英文 ROM 与 32M 中文 ROM，boot-check 双通过；字库容量已实测（见 §3.5）。
...
- ⬜ **中文 ROM 实机试玩**：用 mGBA 打开 32M ROM，确认中文文本实际渲染（不溢出/不断行/不乱码）。**这是最后一道人工验证**，见下方
...
ROM 在 WSL 文件系统内，Windows 侧通过 UNC 路径访问：

```
\\wsl$\Ubuntu-24.04\home\shanhe\projects\fireemblem8-expansion\build\expansion-modern\release\aapcs\
```

用 **mGBA** 打开该目录下的 `fireemblem8.gba`（32 MB，中文版，SHA1 `508f6109…`）。

**观察清单**：
1. 标题画面文字是否为中文
2. 进入游戏后菜单项（道具/状态/存档）是否中文
3. **重点**：对话框文本有无**超出边框**、**断行位置怪异**、**豆腐块（缺字方框）**
4. 战斗预测面板数字是否正常

若出现豆腐块 → 说明该字不在 3,842 字形池内，需扩充 `texts/locales/zh-Hans/` 字符集后重新 `make localization-generate`。

> 建议把 ROM 拷到 Windows 侧再玩（避免 UNC 路径下模拟器 IO 卡顿）：
> ```bash
> # WSL 内
> cp build/expansion-modern/release/aapcs/fireemblem8.gba \
>    /mnt/d/workbuddy/shanhe-cn-32m.gba
> ```
