...
| 构建失败但看不出原因 | **先跑 `bash tools/wsl/6-diagnose-build.sh`**（一次性体检子模块/工具链/依赖/日志/产物，直接给根因）。或手动 `grep -nE "error:\|fatal error\|undefined reference" ~/shanhe-logs/build-*.log` |
| **报 `can't open tools/gbagfx/gbagfx.s for reading`** | **子模块未初始化**（最高频根因）。`git clone --recursive` 在国内网络下会静默失败。修复：`git config --global http.proxy http://127.0.0.1:7897` → `git submodule update --init --recursive --depth 1`。若子模块用 `git://` 协议则需 `git config --global url."https://github.com/".insteadOf "git://github.com/"` |
