...
> 状态：**v0.9，修正"文本进 ROM"的模型（两条通道 → 三条来源 + 优先级）**。在 v0.8 之上：**§4.4 重写为"三条来源 + 优先级"**（撤销 v0.8 的"通道 A 不进 ROM"结论——经 ELF 段表复核，A 经 `.locale_data` 段进 ROM 且**优先级高于 B**），并新增产物层验证步骤；同步新增 **§4.6 存档兼容纪元**、**§5.6 引擎级地雷**（Proc 64 槽无越界检查等）。
...
3. **交付前的排查动作**：遇到"只有带存档才崩、无存档正常"这类现象，**先把旧 `.sav` 移开再复现一次**——这是区分"存档不兼容"与"引擎 bug"的最快一刀（本项目 2026-09-25 的 `Menu_OnInit` 野指针崩溃就是这么定位到"与 `.sav` 强相关"的）。
