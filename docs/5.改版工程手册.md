...
> 状态：**v0.10，修正"文本进 ROM"的模型（两条通道 → 三条来源 + 优先级），并定位第一个实机崩溃**。在 v0.8 之上：**§4.4 重写为"三条来源 + 优先级"**（撤销 v0.8 的"通道 A 不进 ROM"结论——经 ELF 段表复核，A 经 `.locale_data` 段进 ROM 且**优先级高于 B**），并新增产物层验证步骤；新增 **§4.6 存档兼容纪元**、**§5.6 引擎级地雷**（Proc 64 槽无越界检查等）、**§5.7 教学关卡 `Menu_OnInit` 野指针崩溃的根因链**（与中文化无关，已确证中文版/英文版同样崩）。**v0.10 补记：该崩溃已定位到根因链（§5.7），并修正此前"与 `.sav` 强相关"的错误结论。**
...
- **备选**：在本仓库走内容外置补丁通道打**最小本地补丁**（`Menu_OnInit` 加 `if (proc->itemCount == 0) return;`；填充循环加 `itemCount < MENU_ITEM_MAX` 保护），并在本手册登记为**已知偏离**。
- **临时绕过**：调整教学章的事件脚本，使其不会把单位菜单项全部隐藏。
