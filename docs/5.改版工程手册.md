...
> 状态：**v0.6，环境·构建·实机游玩全链路跑通，方案 B 已落地**。M1 骨架完成（`shanhe-build.sh` 六步 + 六条验收全过）；新增 §3.6 方案 B 全链路、§4.1 改写为 B' 回填。
...
| 项 | 位置 |
|---|---|
| ROM（WSL 内） | `~/projects/fireemblem8-expansion/build/expansion-modern/release/aapcs/fireemblem8.gba` |
| ROM（Windows 访问） | `\\wsl$\Ubuntu-24.04\home\shanhe\projects\fireemblem8-expansion\build\expansion-modern\release\aapcs\fireemblem8.gba` |
| 直接启动 | `D:\workbuddy\shanhe-rom\启动山河烬中文版.cmd` |
