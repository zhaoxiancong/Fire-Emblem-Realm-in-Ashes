...
> 状态：**v0.8，原创内容进 ROM 的两条通道全部打通**。在 v0.7（环境·构建·实机游玩全链路 + 方案 B 落地）之上，新增 **§3.7 原创汉字补字管线**（8 字入字库，实测量取为证）、**§4.4 改写为"两条文本通道"**（补上真正进 ROM 的消息表通道）、**§5.5 ROM 内文本验证法**（Huffman 解压比对）。
...
| 做一章关卡 | §4.2 |
| 改战斗公式 | §4.3 |
| **加中文文本（两条通道）** | **§4.4（⚠️ 只做通道 A 则 ROM 里还是英文）** |
| 验证改动没弄坏东西 | §5 验证与测试 |
...
### 4.4 加中文文本（**两条通道，别只做一条**）

> **M2 血泪教训**：框架有**两条作用完全不同的文本通道**。只做通道 A，会出现「**审计全绿、报告漂亮、ROM 里还是英文**」的假成功。

| 通道 | 文件 | 作用 | 进 ROM 吗 | 定位键 |
|---|---|---|---|---|
| **A · 本地化台账** | `texts/locales/<locale>/indexed.txt` | 审计 / 宽度校验 / 台账 / 泄漏检查 | ❌ **不进** | FE8J **source index**（`#0xNNNN`） |
| **B · 消息表** | `texts/texts.txt` | **真正被 Makefile 编译进 ROM** | ✅ **进** | FE8U **target id**（`## MSG_<HEX>`） |

编译链路（`Makefile:562`）：`src/msg_data.c ← $(TEXT_PROCESS) $(TEXT_MAIN) $(TEXT_DEFS) $@ $(TEXT_HEADER) utf8`，其中 `TEXT_MAIN := texts/texts.txt`，共 **3414 条**消息。

**通道 A 的内容外置补丁**：`content/texts/locales/indexed_overrides.zh-Hans.json`（由 `shanhe-build.sh` 的 **3b' 步**铺设）。
**通道 B 的内容外置补丁**：`content/texts/msg_overrides.zh-Hans.json`（由 **3b'' 步**合并，**这才是让 ROM 里出现中文的那一步**）。
...
3. **通道 A 与 B 的键不同**，写错通道 A 的键不会报错，只会静默不生效（因为它不进 ROM）。
...
**通道 B 的验证** → 见 **§5.4**（ROM 内文本验证法）。**通道 A 全绿不代表通道 B 生效**——这是本节最重要的一句话。
...
> ⚠️ **headless 自动导航的坑**（实测多次）：FE8 序章对话极长（纯 A 需 ~10 万帧才进战斗地图）；插 `Start` 键会在战斗地图上反复开关菜单导致 `Jumped to invalid address` 崩溃。**要"看屏幕效果"，最可靠的是双击 `启动山河烬中文版.cmd` 用真 mGBA 目视**，而不是硬啃 headless 导航。
